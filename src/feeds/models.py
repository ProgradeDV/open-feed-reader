...
LOGGER = logging.getLogger("Feed Models")


class Source(models.Model):
    """Data describing the source of a feed and the fetch behavior."""

    name          = models.CharField(max_length=255, blank=True, default="")
    title         = models.CharField(max_length=255, blank=True, default="")
    subtitle      = models.TextField(max_length=255, blank=True, default="")
    site_url      = models.URLField(max_length=255, blank=True, default="") # link
    feed_url      = models.URLField(max_length=512, unique=True) # href
    image_url     = models.URLField(max_length=512, blank=True, default="") # image.href
    icon_url      = models.URLField(max_length=512, blank=True, default="") # icon
    author        = models.CharField(max_length=255, blank=True, default="") # author
    description   = models.TextField(default="", blank=True) # info

    # === due tracking ===
    # the last time it was fetched
    last_feched = models.DateTimeField(blank=True, default="")
    # the last time there was a succsessfull query
    last_success = models.DateTimeField(blank=True, default="")
    ...
    due_fetch = models.DateTimeField(default=datetime.datetime(1900, 1, 1, tzinfo=datetime.UTC))
    ...
    # this is not a datetime field because this value is from the fetch metadata
...
class Entry(models.Model):
    ...
    guid          = models.CharField(max_length=512, blank=True, default="", db_index=True) # id
    ...
    def title_url_encoded(self) -> str:
        ...
        try:
            ...
            LOGGER.exception("Failed to url encode title of entry %s", self.id)
            return ""

        if len(ret) > 2:  # noqa: PLR2004
            return ret[2:]
        return ret
...
class Enclosure(models.Model):
    ...
    def __str__(self) -> str:
        """Enclosure display info."""
        return f"{self.entry.title}[{self.type}]"
